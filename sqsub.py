...
class JobTracker(Daemon):
    """Daemon which tracks job status in the background."""
    def __init__(self, jobid, logfile, stdout):
        """jobid and logfile are strings!"""
    ...
    def offline_nodes(self, nodes_list):
        """Ping all nodes. Returns None if OK; otherwise returns the first bad (offline) node."""
        # Check health of each node by pinging it
